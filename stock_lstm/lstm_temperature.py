...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 11 13:23:44 2017
...
def denormalise_windows(normalised_data):
    window_data = []
    for normalised in normalised_data:
        denormalised_window = [(float(normalised[0])*(float(n) +1)) for n in normalised]
        window_data.append(denormalised_window)
    return window_data
...
if __name__=='__main__':
    ...
    X_train, y_train, X_test, y_test = load_data('temperature_example.csv', seq_len, True)
...
#    checkpoint = ModelCheckpoint("models/model_" + model_name + "_" + holdout_txt + "_e" + "{epoch:02d}-{val_loss:.4f}.hd5", monitor='val_loss', verbose=1, save_best_only=not train_full_set, save_weights_only=False, mode='auto', period=1)
    checkpoint_fixed_name = ModelCheckpoint("model_weight_best_e{epoch:02d}-{val_loss:.4f}.hd5", monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
#    model.fit_generator(train_gen, len(train_files_res) / 1, epochs, validation_data=holdout_gen, nb_val_samples=len(holdout_files_res) / 1, callbacks=[checkpoint, checkpoint_fixed_name, learnrate_scheduler])
    ...
    model.fit(
	    ...
	    batch_size=512,
	    epochs=epochs,
	    validation_split=0.05, callbacks=[checkpoint_fixed_name])
